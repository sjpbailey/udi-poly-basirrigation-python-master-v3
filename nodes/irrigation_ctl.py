...
class Controller(udi_interface.Node):
    ...
    def createChildren(self, how_many):
        ...
        LOGGER.info('Creating {} Irrigation Zone'.format(how_many))
        for i in range(0, how_many):
            address = 'zone_{}'.format(i)
            title = 'Irrigation {}'.format(i)            
            ip = self.Parameters.irrip_0
            LOGGER.info(ip)
            ip1 = self.Parameters.irrip_1  
            LOGGER.info(ip1)
            ip2 = self.Parameters.irrip_2
            LOGGER.info(ip2)
            ip3 = self.Parameters.irrip_3  
            LOGGER.info(ip3)
            ip4 = self.Parameters.irrip_4
            LOGGER.info(ip4)
            ip5 = self.Parameters.irrip_5  
            LOGGER.info(ip5)
            node = irrigation_zone.IrrigationNode(self.poly, self.address, address, title, ip, ip1, ip2, ip3, ip4, ip5, self.bc )
